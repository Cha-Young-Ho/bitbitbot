...
func (cbw *CoinbaseWorker) executeSellOrder(ctx context.Context) {
	...
	if orderResp != nil {
		if orderResp.Success && orderResp.SuccessResponse != nil {
			cbw.storage.AddLog("success",
				fmt.Sprintf("매도 주문 성공: 주문번호=%s, 가격=%.8f, 수량=%.8f",
					orderResp.SuccessResponse.OrderID, cbw.config.SellPrice, cbw.config.SellAmount),
				cbw.config.Exchange, cbw.config.Symbol)
		} else {
			cbw.storage.AddLog("error", fmt.Sprintf("매도 주문 실패 응답: %+v", orderResp.ErrorResponse), cbw.config.Exchange, cbw.config.Symbol)
		}
	}
}
